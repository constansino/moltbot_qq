...
const IdListInputSchema = z.preprocess((value) => {
...
export const QQConfigSchema = z.object({
  ...
  admins: IdListInputSchema.describe("管理员QQ号（直接填数字，不用加引号）。支持逗号/空格/换行分隔。示例：1838552185,123456789"),
  ...
  allowedGroups: IdListInputSchema.describe("允许响应的群号白名单（直接填数字）。支持逗号/空格/换行。示例：883766069 123456789"),
  blockedUsers: IdListInputSchema.describe("用户黑名单QQ号（直接填数字）。支持逗号/空格/换行。示例：10001,10002"),
  historyLimit: NumberInputSchema(0).describe("群历史注入条数。默认0（推荐，依赖会话系统）；需强保留原文时可设 3~5。"),
  keywordTriggers: KeywordTriggersSchema.describe("关键词触发（无需@）。可填数组或文本（逗号/空格/换行分隔）。示例：小助手, 帮我"),
