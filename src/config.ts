...
export const QQConfigSchema = z.object({
  ...
  showProcessingStatus: BooleanInputSchema(true).describe("忙碌状态可视化开关。开启后，机器人在群里处理任务时会临时把自己的群名片改成“(输入中)”后缀。"),
  processingStatusDelayMs: NumberInputSchema(500).describe("触发“输入中”群名片后缀的延迟（毫秒）。"),
  processingStatusIntervalMs: NumberInputSchema(0).describe("保留字段（当前未使用）。"),
  processingStatusText: z.preprocess((value) => normalizeLooseString(value), z.string().optional().default("输入中")).describe("忙碌后缀文本。示例：输入中。"),
